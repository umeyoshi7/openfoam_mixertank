...
Vertex AI コンテナが使用するファイルのみアップロードします。
メッシュ生成用ファイルや計算済みタイムディレクトリは**不要**です。
...
**アップロード不要なファイル（LK-1_HD0.45/）:**

| ファイル/ディレクトリ | 除外理由 |
|--------------------|---------|
| `0.backup/` | mapFields 実行時にコンテナ内で自動生成 |
| `constant/triSurface/` | snappyHexMesh 用、メッシュ生成済みのため不要 |
| `constant/extendedFeatureEdgeMesh/` | snappyHexMesh 用、メッシュ生成済みのため不要 |
| `system/blockMeshDict` | メッシュ生成用、不要 |
| `system/snappyHexMeshDict` | メッシュ生成用、不要 |
| `system/surfaceFeatureExtractDict` | メッシュ生成用、不要 |
| `system/createBafflesDict` | メッシュ生成用、不要 |
| `log.*` | 実行ログ（実行時生成） |
| `10/`, `20/`, ... | 計算済みタイムディレクトリ |
| `processor*/` | 並列分割結果 |
...
### LK-1_HD0.45_MRF/ アップロード対象ファイル一覧

```
LK-1_HD0.45_MRF/
├── 0.orig/                      ← restore0Dir のコピー元（必須）
│   ├── U
│   ├── p
│   ├── k
│   ├── epsilon
...
│     （entrypoint.sh の Step 3 でコンテナ内に自動再作成）
└── system/
    ├── controlDict
    ├── decomposeParDict
    ├── fvSchemes
...
**アップロード不要なファイル（LK-1_HD0.45_MRF/）:**

| ファイル/ディレクトリ | 除外理由 |
|--------------------|---------|
| `0/` | `restore0Dir` が `0.orig/` からコンテナ内で自動生成 |
| `constant/polyMesh` | シンボリックリンク（実体なし）、entrypoint.sh が再作成 |
| `Allrun`, `Allclean` | ローカル実行用スクリプト、コンテナでは不使用 |
| `mapToTransient.sh` | ローカル実行用スクリプト、コンテナでは不使用 |
| `log.*` | 実行ログ（実行時生成） |
| `10/`, `20/`, ... | 計算済みタイムディレクトリ |
| `processor*/` | 並列分割結果 |
...
```dockerfile
# ベースイメージ: openfoam.org 公式イメージ (OpenFOAM v11 + ParaView 5.10)
# /opt/openfoam11/ に OpenFOAM がインストール済み
FROM openfoam/openfoam11-paraview510
```

- **Google Cloud CLI** (`google-cloud-cli`) をインストール
  → `gsutil` コマンドによる GCS アクセス（ケースダウンロード・結果アップロード）に使用
- `python3` は `google-cloud-cli` の依存として維持（pip パッケージは不使用）
- `entrypoint.sh` をコンテナ内にコピーし、自動実行
...
`cases/` フォルダを作成し、上記「GCS にアップロードするファイル」に従ってアップロードします。

> **重要**: `LK-1_HD0.45_MRF/constant/polyMesh` はシンボリックリンクのため
> アップロードしないこと。コンテナ起動時に `entrypoint.sh` が自動再作成します。
...
Artifact Registry への Docker 認証を設定したうえで、以下を実行してください:

- イメージ URI フォーマット: `REGION-docker.pkg.dev/PROJECT_ID/REPO/IMAGE:TAG`
  - 例: `asia-northeast1-docker.pkg.dev/<PROJECT_ID>/openfoam/openfoam-ami:latest`
- `docker build --platform linux/amd64 -t <IMAGE_URI> docker/` でビルド
- `docker push <IMAGE_URI>` で Artifact Registry へ Push

### 3-4. イメージ確認（Console GUI）
...
| `decomposeParDict` の並列数 | `Allrun` の `-np` 引数と `entrypoint.sh` の `NCORES` も合わせて変更 |
...
# -case が target（転写先）、最後の引数が source（転写元）
...
GCS に `LK-1_HD0.45/constant/polyMesh/` が正しくアップロードされていない可能性。
GCS コンソールで `cases/LK-1_HD0.45/constant/polyMesh/` フォルダにファイルが存在するか確認。

ローカルで発生する場合はシンボリックリンクが壊れています:

```sh
rm LK-1_HD0.45_MRF/constant/polyMesh
ln -s ../../LK-1_HD0.45/constant/polyMesh LK-1_HD0.45_MRF/constant/polyMesh
```
...
`mapToTransient.sh` 内のコマンドの引数順序を確認してください（上記「注意事項」参照）。
