...
| 設定項目 | LKHD045 (pimpleFoam) | LKHD045MRF (simpleFoam) |
|---------|--------------------------|-------------------------------|
...
│
├── vertex_ai/                   Vertex AI Custom Job 定義
...
│   └── system/
│
└── LKHD045MRF/             定常ケース (simpleFoam + MRF) ← 初期値生成用
    ├── 0.orig/                  初期条件（restore0Dir のコピー元）
    ├── constant/
    │   ├── polyMesh → ../../LKHD045/constant/polyMesh  (シンボリックリンク)
    │   └── MRFProperties
    ├── system/
...
└── results/                       ← ソルバージョブの出力（自動生成）
    ...
    └── LKHD045_<TIMESTAMP>/   タイムスタンプ付き結果（イミュータブル）
        ├── 0/                     初期条件（mapFields 後）
...
| ファイル/ディレクトリ | 除外理由 |
|--------------------|---------|
| `LKHD045/constant/polyMesh/` | メッシュ生成ジョブが生成し GCS の `mesh/` に保存 |
| `LKHD045MRF/constant/polyMesh` | シンボリックリンク（実体なし）、entrypoint_solver.sh が再作成 |
...
スクリプトが参照する環境変数は実行環境に応じて以下の場所で定義します。
...
```bash
cp .env.example .env
# .env を編集して GCS_BUCKET に実際のバケット名を入力
```

```bash
# メッシュ生成ジョブのテスト実行
docker run --env-file .env <IMAGE_URI> /entrypoint_mesh.sh

# ソルバージョブのテスト実行
docker run --env-file .env <IMAGE_URI> /entrypoint_solver.sh
```

`.env` は `.gitignore` 済みのためコミットされません。

### Vertex AI Custom Job（`gcloud` CLI）

`vertex_ai/job_mesh.yaml` と `vertex_ai/job_solver.yaml` の以下 2 箇所を書き換えます。

| プレースホルダー | 変更内容 |
|----------------|---------|
| `<PROJECT_ID>` | GCP プロジェクト ID |
| `your-openfoam-bucket` | GCS バケット名 |
...
| `GCS_BUCKET` | **必須** | — | 両方 |
| `NCORES` | 任意 | `4` | 両方 |
| `CASE_NAME` | 任意 | `LKHD045` | mesh |
| `GCS_MESH_PREFIX` | 任意 | `mesh` | mesh |
| `GCS_RESULT_PREFIX` | 任意 | `results` | solver |
| `MRF_END_TIME` | 任意 | `3000` | solver |
| `GCS_MESH_PATH` | 任意 | `mesh/latest.txt` から自動取得 | solver |
...
## Step 4: Vertex AI Custom Training Job の作成と実行
...
必要に応じて `NCORES`（マシンの vCPU 数以下）、`MRF_END_TIME` なども調整します。
...
完了すると `gs://<GCS_BUCKET>/mesh/latest.txt` が生成され、ソルバーが自動参照します。
...
CLI の代わりに GCP コンソールからも実行できます。

GCP コンソール → **Vertex AI** → **Training** → **カスタムジョブ** → **作成**

- **コンテナイメージ URI**: `vertex_ai/job_*.yaml` の `imageUri` の値を入力
- **環境変数**: 上記「環境変数一覧」の値を GUI フォームに入力
...
`LKHD045_<TIMESTAMP>/` ディレクトリに計算結果が格納されています。
...
2. `LKHD045/0/` を `LKHD045/0.backup/` にバックアップ
...
grep -A4 "^internalField" ../LKHD045/0/U | head -6

# BC 型が pimpleFoam 用のまま保持されていることを確認
grep -A2 "impeller_HD0.45" ../LKHD045/0/U
...
# 正しい（LKHD045MRF/ から実行、MRF → pimpleFoam）
mapFields . -consistent -sourceTime 3000 -case ../LKHD045

# 誤り（引数逆転、pimpleFoam → MRF になる）
mapFields ../LKHD045 -consistent -sourceTime 3000 -case .
```
...
```
constant/polyMesh → ../../LKHD045/constant/polyMesh
                    ^^
