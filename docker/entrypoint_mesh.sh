...
#   MAX_GLOBAL_CELLS  snappyHexMesh の最大セル数 (デフォルト: 3000000 [本番])
#                     ローカルテストでは 300000 程度を推奨
#------------------------------------------------------------------------------
set -euo pipefail
...
MAX_GLOBAL_CELLS="${MAX_GLOBAL_CELLS:-3000000}"
WORKSPACE="/workspace"
...
# ---------------------------------------------------------------------------
echo ""
echo "[Step 5] snappyHexMesh 実行 (NCORES=${NCORES}, maxGlobalCells=${MAX_GLOBAL_CELLS})"
cd "${CASE_DIR}"
foamDictionary \
